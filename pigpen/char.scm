...
(define <pigpen-char>
  (make-vtable "prprpr"
               (lambda (struct port)
                 (format port "#<pigpen-char ~a ~@c ~a>"
                         (struct-ref struct 0) ;cipher-name
                         (struct-ref struct 1) ;char
                         (number->string (object-address struct) 16)))))

(define (make-pigpen-char cipher-name ch lst)
  "Make a pigpen char that represents a char CH based on a list LST of
strings."
  (make-struct/no-tail <pigpen-char> cipher-name ch lst))
...
(define (char->pigpen-char ch cipher)
  ...
  (let ((cipher-name (get-cipher-name cipher)))
    (cond ((not (char? ch))
           (error "Wrong type (expecting char)" ch))
          ((char=? ch #\nul)
           (make-pigpen-char cipher-name ch (list "" "" "")))
          ((assoc-ref (get-cipher-alphabet cipher) (char-downcase ch)) =>
           (cut make-pigpen-char cipher-name ch <>))
          (else
           (make-pigpen-char cipher-name
                             ch
                             (list
                              "     "
...
LST is expected to be a list of 3 elements."
  (if (= (length lst) 3)
      (let* ((alphabet    (get-cipher-alphabet cipher))
             (cipher-name (get-cipher-name     cipher))
             (m (find (lambda (e)
                       (let ((l (cdr e)))
                     ...
                     alphabet)))
        (if m
            (make-pigpen-char cipher-name (car m) lst)
            (let ((ch (string-trim (list-ref lst 1))))
              (if (not (string-null? ch))
                  (make-pigpen-char cipher-name (string-ref ch 0) lst)
...
(define (pigpen-char->string pch)
  (if (pigpen-char? pch)
      (let ((pchar-list (struct-ref pch 2)))
